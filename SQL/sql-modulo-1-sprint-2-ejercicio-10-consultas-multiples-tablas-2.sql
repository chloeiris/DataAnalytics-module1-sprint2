...
SELECT orders.order_number AS OrderID, customers.customer_name AS CompanyName, orders.order_date AS OrderDate
FROM customers
LEFT JOIN orders
...
SELECT customers.customer_name AS CompanyName, customers.contact_first_name AS ContactName, orders.order_number AS OrderID, orders.order_date AS OrderDate, customers.country
FROM customers
LEFT JOIN orders
USING (customer_number)
...
-- La directora es Diane Murphy, de San Francisco, puesto que no tiene nadie a quien reportar
...
SELECT customers.customer_name AS CompanyName, orders.order_number AS OrderID, orders.order_date AS OrderDate
FROM customers
LEFT JOIN orders
USING (customer_number)
UNION
